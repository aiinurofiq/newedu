<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex justify-content-between mb-4">
        <h4 class="fw-bold mb-2"><span class="text-muted fw-light">Home /</span> Category</h4>
        <button wire:click="$set('action', 'C')"
            class="create-new btn btn-primary {{ $action == 'R' ? '' : 'd-none' }}"><i
                class="ti ti-plus me-sm-1"></i><span class="fw-bold">Buat Category</span></button>
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li><a wire:click="edit({{ $item->id }})" class="dropdown-item"
                                                ...
                                                <span class="align-middle">Edit Category</span></a></li>
                                        <li><a wire:click="delete({{ $item->id }})" class="dropdown-item"
                                                href="javascript:void(0);">
                                                <i class="ti ti-trash me-2 ti-sm"></i>
                                                <span class="align-middle">Delete Category</span></a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
    ...
    <div class="card {{ $action == 'C' || $action == 'U' ? '' : 'd-none' }}">
        <h5 class="card-header">{{ $editknowledge ? 'Edit' : 'Tambah' }} Category </h5>
        <div class="card-body">
            <form wire:submit.prevent="confirm" id='formreset' class="row g-3">
                <div class="col-md-12">
                    ...
                    <input wire:model="category" type="text"
                        class="form-control @error('category') is-invalid @enderror" placeholder="Input Category" />
                    @error('category')
